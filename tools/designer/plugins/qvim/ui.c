    ...
    long    wtime;	    /* don't use "time", MIPS cannot handle it */
{
	...
	if (maxlen >= ta_len - ta_off)
	{
	    ...
	    vim_free(ta_str);
	    ta_str = NULL;
	    printf("ui_inchar: returning 1\n");
	    return ta_len;
	}
	mch_memmove(buf, ta_str + ta_off, (size_t)maxlen);
	ta_off += maxlen;
	printf("ui_inchar: returning 2\n");
	return maxlen;
    }
#endif
    ...
    {
	printf("ui_inchar: gui starting or not in use\n");
	buf[0] = CR;
	return 1;
    }
#endif
    ...
    if (gui.in_use)
    {
	printf("calling gui_wait_for_chars\n");
	if (!gui_wait_for_chars(wtime))
	    return 0;
	printf("returning read_from_input_buf\n");
