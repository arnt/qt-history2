...
{
    qWarning( "%s: %s", yyFileName.latin1(), message.latin1() );
}
...
void Dlg2Ui::emitWidgetBody( const QDomElement& e, bool layouted )
{
	...
	if ( isWidgetType(n.toElement()) ) {
		...
		if ( tagName == QString("Alignement") ||
		    ...
		    error( QString("Unknown property '%1'.").arg(tagName) );
		}
	    } else {
...
bool Dlg2Ui::checkTagName( const QDomElement& e, const QString& tagName )
{
    ...
    if ( !ok )
	error( QString("Expected tag '%1', met '%2'").arg(tagName)
	       .arg(e.tagName()) );
    return ok;
}
...
void Dlg2Ui::matchDialogCommon( const QDomElement& dialogCommon )
{
	...
	if ( tagName == QString("SourceDir") ) {
	    ...
	    /*
	      Unfortunately, Qt Designer does not allow to subclass a
	      custom widget.
	    */
...
void Dlg2Ui::matchLayoutWidget( const QDomElement& layoutWidget )
{
	...
	if ( w == yyWidgetMap.end() ) {
	    error( QString("Unknown widget '%1' in layout.").arg(widget) );
	} else {
...
void Dlg2Ui::matchWidget( const QDomElement& widget )
{
	...
	error( QString("There are two widgets named '%1'.").arg(name) );
...
QStringList Dlg2Ui::convertQtArchitectDlgFile( const QString& fileName )
{
	...
	error( QString("File is not a Qt Architect XML file.") );
	return QStringList();
    }
    if ( root.attributeNode("type").value() != QString("Dialog") ) {
	error( QString("File is not a Qt Architect XML Dialog file.") );
	return QStringList();
    }
	...
	qFatal( "dlg2ui: Could not open output file '%s'",
		outFileName.latin1() );
