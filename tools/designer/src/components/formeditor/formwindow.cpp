...
void FormWindow::addFormAction(QAction *action)
{
    if (mainContainer() == 0)
        return;

    if (action->parent() == mainContainer())
        return;

    action->setParent(mainContainer());

    core()->metaDataBase()->add(action);
    QDesignerPropertySheetExtension *sheet
        = qt_extension<QDesignerPropertySheetExtension*>(core()->extensionManager(), action);
    sheet->setChanged(sheet->indexOf("objectName"), true);
    sheet->setChanged(sheet->indexOf("text"), true);
    emit formActionsChanged();
}

void FormWindow::removeFormAction(QAction *action)
{
    if (mainContainer() == 0)
        return;
...
QList<QAction*> FormWindow::formActionList() const
{
    QList<QAction*> result = qFindChildren<QAction*>(mainContainer());

    int i = 0;
    while (i < result.size()) {
        QAction *action = result.at(i);
        if (core()->metaDataBase()->item(action) == 0 || action->isSeparator())
            result.removeAt(i);
        else
            ++i;
    }
