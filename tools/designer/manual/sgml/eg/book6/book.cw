<!DOCTYPE CW><CW>
<customwidgets>
    <customwidget>
        <class>BookSqlTableBase</class>
        <header location="local">booksqltablebase.h</header>
        <sizehint>
            <width>-1</width>
            <height>-1</height>
        </sizehint>
        <container>0</container>
        <sizepolicy>
            <hordata>7</hordata>
            <verdata>7</verdata>
        </sizepolicy>
        <pixmap>
            <data format="XPM.GZ" length="646"></data>
        </pixmap>
        <slot access="public">slotNewAuthor(const QSqlRecord *)</slot>
    </customwidget>
    <customwidget>
        <class>AuthorComboBox</class>
        <header location="local">authorcombobox.h</header>
        <sizehint>
            <width>-1</width>
            <height>-1</height>
        </sizehint>
        <container>0</container>
        <sizepolicy>
            <hordata>5</hordata>
            <verdata>5</verdata>
        </sizepolicy>
        <pixmap>
            <data format="XPM.GZ" length="646"></data>
        </pixmap>
    </customwidget>
    <customwidget>
        <class>FormatComboBox</class>
        <header location="local">formatcombobox.h</header>
        <sizehint>
            <width>-1</width>
            <height>-1</height>
        </sizehint>
        <container>0</container>
        <sizepolicy>
            <hordata>5</hordata>
            <verdata>5</verdata>
        </sizepolicy>
        <pixmap>
            <data format="XPM.GZ" length="646"></data>
        </pixmap>
    </customwidget>
</customwidgets>
</CW>
