...
namespace QMsNet
{
	...
	public void loadProject()
	{
		...
		QMNCommands.Say( "Generating new .vcproj of .pro file" );
	    ...
	    try {
		QMNCommands.Say( "Adding project to Solution" );
	    ...
	    QMNCommands.Say( "-- loadProject - done" );
	}

	public void saveProject()
	{
	    try {
		QMNCommands.Say( "-- saveProject" );
		Document doc = Connect.applicationObject.ActiveDocument;
		if ( doc == null ) {
		    QMNCommands.Say( "Cannot save Qt project... No file open" );
		    return;
		}
		...
		QMNCommands.Say( "Saving first configuration - " + cfg.Name );
	    ...
	    QMNCommands.Say( "-- saveProject - done" );
	}
