...
/*!
    \page dbusxml2cpp.html
    \title QtDBus XML compiler (dbusxml2cpp)
    ...
    \c dbusxml2dcpp has two modes of operation, that correspond to the two possible outputs it can
    ...
    The \c dbusxml2dcpp tool is not meant to be run every time you compile your
    application. Instead, it's meant to be used when developing the code or when the interface
    changes.

    The adaptor classes generated by \c dbusxml2cpp are just a skeleton that must be completed. It
