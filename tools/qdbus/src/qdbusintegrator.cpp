                                       ...
                                       const QDBusMessage &msg)
{
    ...
    for (int i = 0; i < n; ++i)
        if (metaTypes.at(i + 1) != msg.at(i).userType() &&
            msg.at(i).userType() != qMetaTypeId<QDBusArgument>())
            return 0;           // no match
...
void QDBusConnectionPrivate::deliverCall(const CallDeliveryEvent& data) const
{
    ...
    for (i = 1; i <= msg.count(); ++i) {
        ...
        else if (msg.at(i - 1).userType() == qMetaTypeId<QDBusArgument>()) {
            // convert to what the function expects
            void *null = 0;
            auxParameters.append(QVariant(id, null));
            
            const QDBusArgument &in =
                *reinterpret_cast<const QDBusArgument *>(msg.at(i - 1).constData());
                                                   ...
                                                   int sendMode, int timeout)
{
	...
	QDBusMessage amsg;
	if (isServiceRegistered(message.service())) {
            qWarning("QDBusConnection: error: the sender and receiver are in the same thread");
            dbus_message_unref(msg);
	    //### we should do the call internally here to avoid blocking
	    amsg.sendError(QString::fromLocal8Bit("InternalError"),
	    ...
	    handleError();
            ...
            amsg = QDBusMessagePrivate::fromDBusMessage(reply, QDBusConnection(name));
...
bool QDBusConnectionPrivate::isServiceRegistered(const QString &serviceName) const
{
