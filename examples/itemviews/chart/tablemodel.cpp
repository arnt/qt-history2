...
TableModel::TableModel(int rows, int columns, QObject *parent)
    : QAbstractTableModel(parent), tableColumns(columns)
{
    QStringList newList;
                                ...
                                int role) const
{
    ...
    if (orientation == Qt::Horizontal)
        switch (section) {
            case 0: return QString("Label");
            case 1: return QString("Amount");
...
{
    int rows = rowCount();
    int removed = qMin(columns, columnCount() - position);
    emit columnsAboutToBeRemoved(QModelIndex(), position, position+removed-1);

    for (int row = 0; row < rows; ++row) {
        for (int column = 0; column < removed; ++column) {
