...
template<class RowT, class ColT, class CellT> class QTablePrivate : public QShared
{
 public:
  QTablePrivate() : QShared(){ row_count = 0; col_count = 0; }
  QTablePrivate( uint _rows, uint _cols ) : QShared() {
  ...
  QTablePrivate( const QTablePrivate& _t ) : QShared() {
...
template<class RowT, class ColT, class CellT> class QTable
{
private:
  typedef QTablePrivate<RowT,ColT,CellT> Priv;
...
public:
   ...
   */
  QTable() { sh = new Priv; }
  QTable( uint _rows, uint _cols ) { sh = new Priv( _rows, _cols ); }
  QTable( const QTable& _t ) { sh = _t.sh; sh->ref(); }
  ~QTable() { if ( sh->deref() ) delete sh; }
