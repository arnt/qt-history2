...
QFontCache::QFontCache() :
    ...
    QCache<FontEngineIface>(qtFontCacheMin, qtFontCacheSize),
...
bool QFontCache::insert(const QString &key, const FontEngineIface *qfs, int cost)
{

#ifdef QFONTCACHE_DEBUG
    qDebug("QFC::insert: inserting %p w/ cost %d\n%s", qfs, cost,
	   (qfs == (FontEngineIface *) -1) ? "negative cache item" : qfs->name.data());
#endif // QFONTCACHE_DEBUG
    ...
    bool ret = QCache<FontEngineIface>::insert(key, qfs, cost);
...
#ifdef Q_WS_MAC
template<> inline void QCache<FontEngineIface>::deleteItem( QPtrCollection::Item d )
#else
void QFontCache::deleteItem(Item d)
#endif
{

    FontEngineIface *qfs = (FontEngineIface *) d;
	...
	qWarning( "Multiple QCache<FontEngineIface> exist." );
