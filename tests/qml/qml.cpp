...
{
  if (style.name() == "img")
    return new QMLImage(attr, context);
  return 0;
}
...
QMLImage::QMLImage(const QDict<QString> &attr, const QMLContext &context)
  : QMLCustomNode()
{
//   debug("create qmlimage");
  QString* imageName = attr["source"];
  if (imageName) {
    pm = context.image( *imageName );
    width = pm.width();
    height = pm.height();
    if (pm.isNull()) {
      width = height = 10;
...
/*!  Bind the given name to a pixmap. The pixmap can be accessed with
  image() */
void QMLContext::setImage(const QString& name, const QPixmap& pm)
{
...
/*!  Bind the given name to a document. The document can be accessed with
  document() */
void QMLContext::setDocument(const QString &name, const QString& doc)
{
...
\sa insert()
*/
QString QMLContext::document(const QString &name) const
{
...
{
  context_ = new QMLContext; // for access during parsing only
...
QMLDocument::QMLDocument( const QString &doc, const QMLContext& context)
  :QMLBox( QMLStyleSheet::defaultSheet().defaultStyle() )
{
  context_ = &context; // for access during parsing only
  sheet_ = &QMLStyleSheet::defaultSheet();// for access during parsing only
  init( doc );
}
...
void QMLDocument::parse (QMLContainer* current, QMLNode* lastChild, const QString &doc, int& pos)
{
  ...
  while ( valid && pos < int(doc.length() )) {
	...
	return;
      ...
      QMLNode* emptytag = sheet_->emptyTag(style, attr, *context_);
      if (emptytag) {
	if (valid) {
	  sep |= eatSpace(doc, pos);
	  QMLNode* l = lastChild;
	...
	QMLContainer* tag = sheet_->tag(style, attr, *context_);
...
{
  papcolgrp = mypapcolgrp;
  doc_ = new QMLDocument( txt, *this, styleSheet() );
  ...
  if (doc_->attributes()["bgpixmap"]){
    QPixmap pm = image(*doc_->attributes()["bgpixmap"]);
    if (!pm.isNull())
...
{
  QMLDocument doc ( txt, *this, styleSheet() );
  {
    QPainter p( this );
    doc.resize(&p, w);
  }
...
void QMLView::resizeEvent(QResizeEvent* e)
{
  ...
  resizeContents(currentDocument().width, currentDocument().height);
  updateContents( contentsX(), contentsY(), viewport()->width(), viewport()->height() );
  debug("qmlview resize event %d", e->size().height() );
}
...
//   QPixmap pm("qt.bmp");
//   t.setImage("qt.bmp", pm);
//   t.setImage("marble01.bmp", QPixmap("marble01.bmp"));
