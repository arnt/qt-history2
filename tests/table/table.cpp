...
void Table::paintCell( QPainter* p, int row, int col )
{
    ...
    */
    ...
    int logCol = col;
    if ( head )
	logCol = head->mapToLogical( col );
    p->drawText( 0, 0, w, h, AlignCenter, contents[indexOf(row,logCol)] );
}
...
{
    return head ? head->cellSize( col ) : QTableView::cellWidth();
}
...
/*!
  Sets the header to \a h.
*/

void Table::setHeader( QHeader *h )
{
    if ( head == h )
	return;
    head = h;
