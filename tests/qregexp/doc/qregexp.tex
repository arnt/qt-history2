...
While matching, a ``leftmost-longest'' rule applies:  The first capture zone should be ``leftmost longest,'' and the second one within the limits imposed by the first match.  Thus both \rx{a*(a*)} and \rx{(a*)a*} capture {\tt aaa} on input {\tt aaa}.  This might be unfortunate, but there's no easy way to change that.
...
Back-references are not supported in their whole theoretical complexity.  For example, \rx{a*(a*)b\\1b} matches {\tt aabb} in theory, but not in practice.  Why?  Because when {\tt aab} is matched, the automaton has already decided that {\tt \\1} means {\tt aa}, even though it could had been {\tt a}~or~$\epsilon$.  Parentheses are greedy and obey a ``leftmost-longest match'' rule (look for {\sf isBetterCapture} in the source code).
