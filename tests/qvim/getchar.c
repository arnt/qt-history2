...
vgetc()
{
    ...
    {
	printf("vgetc(): top of loop\n");
	if (mod_mask)		/* no mapping after modifier has been read */
	{
	    ++no_mapping;
	    ++allow_keys;
	}
	printf("vgetc(): calling get or peek\n");
	c = vgetorpeek(TRUE);
	printf("vgetc(): after calling getorpeek\n");
	if (mod_mask)
	{
	    --no_mapping;
	    --allow_keys;
...
safe_vgetc()
{
    ...
    c = vgetc();
    if (c == NUL) {
	printf("c is NUL, getting keystroke\n");
	c = get_keystroke();
    }
...
# endif
     ...
     */
    if (vgetc_busy) {
	printf("vgetorpeek: busy, returning NUL\n");
	return NUL;
    }
 ...
 */
    if (old_char >= 0)
    {
	printf("vgetorpeek: getting char from previously ungotten char\n");
	c = old_char;
	if (advance)
	    old_char = -1;
	return c;
 ...
 */
	printf("vgetorpeek: reading from stuffbuffer\n");
 ...
 */
#ifdef CMDLINE_INFO
		printf("vgetorpeek:getting char from user\n");
		/*
...
#endif
		printf("vgetorpeek: before calling inchar\n");
		...
		if (c < 0) {
		    printf("vgetorpeek: c<0, continuing\n");
		    continue;		/* end of input script reached */
		}
		if (c == NUL)		/* no character available */
		{
		    printf("vgetorpeek: c is NUL, breaking\n");
		    if (!advance)
			break;
		    if (typelen)		/* timed out */
		    {
			printf("vgetorpeek: timed out, continuing\n");
			timedout = TRUE;
			continue;
		    }
		}
    ...
    printf("vgetorpeek: returning char:%c\n", c);
    ...
    long	wait_time;	    /* milli seconds */
{
    ...
    {
	printf("inchar: did not get from script file\n");
	/*
...
#define DUM_LEN MAXMAPLEN * 3 + 3
	...
	printf("inchar: flushing output chars\n");
	/*
	...
	printf("inchar: calling ui_inchar\n");
	...
	printf("inchar: after calling ui_inchar\n");
    }
