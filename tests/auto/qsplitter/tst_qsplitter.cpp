...
void tst_QSplitter::setSizes3()
{
    ...
    for (int i = 0; i < minimumSizes.size(); ++i) {
        ...
        splitter->setCollapsible(w, collapsibleStates.at(i));
    }
...
void tst_QSplitter::setSizes3_data()
{
    ...
    while (!ts.atEnd()) {
        ...
        ts >> childrenCollapse;
        QTest::newRow(dataName) << minimumSizes << splitterSizes << expectedSizes << collapsibleStates << bool(childrenCollapse);
        ts.skipWhiteSpace();
    }
}
...
void tst_QSplitter::setStretchFactor()
{
    ...
    Qt::Orientation orient = Qt::Orientation(orientation); // C++ is not that bright...
