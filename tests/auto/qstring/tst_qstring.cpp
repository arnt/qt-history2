...
//TESTED_CLASS=
//TESTED_FILES=corelib/tools/qstring.h corelib/tools/qstring.cpp
...
void tst_QString::utf8_data()
{
    ...
    str += QChar(0xdbff);
    str += QChar(0xdec4);
    QTest::newRow( "invalid utf8" ) << QByteArray("hello\344h\344\344\366\344a\304")
			  << str;

    str = "Prohl";
    str += QChar(0xdbff);
    str += QChar(0xde00+0x00ed);
    str += QChar(0xdbff);
    str += QChar(0xde00+0x00be);
    str += "e";
    str += QChar(0xdbff);
    str += QChar(0xde00+0x00e8);
    str += " plugin";
    str += QChar(0xdbff);
    str += QChar(0xde00+0x00f9);
    str += " Netscape";

    QTest::newRow("task28417") << QByteArray("Prohl\355\276e\350 plugin\371 Netscape") << str;
...
void tst_QString::lastIndexOf()
{
    ...
    if (from >= -1 && from < haystack.size()) {
        ...
        // I think QString is wrong --Jasmin
        QCOMPARE(haystack.lastIndexOf(QRegExp(QRegExp::escape(needle), cs), from), expected);
        QCOMPARE(haystack.lastIndexOf(QRegExp(needle, cs, QRegExp::FixedString), from), expected);
    }
...
QString fromInvalidUtf8Sequence(const QByteArray &ba)
{
    ...
    for (int i = 0; i < ba.size(); ++i) {
        result += QChar(0xdbff);
        result += QChar(0xde00 + uchar(ba.at(i)));
    }
    return result;
}
