...
#ifndef QT_QSETTINGS_ALWAYS_CASE_SENSITIVE
                                                          , Qt::CaseInsensitive
#endif
                                                          );
...
void tst_QSettings::init()
{
	...
	if (qWinVersion() & Qt::WV_NT_based)
	    system(QString("rmdir /Q /S %1").arg(settingsPath()).latin1());
	else
	    system(QString("deltree /Y %1").arg(settingsPath()).latin1());
    }
...
#ifndef Q_OS_WIN
    system(QString("chmod -R u+rw %1 2> /dev/null").arg(settingsPath()).latin1());
    system(QString("rm -fr %1 2> /dev/null").arg(settingsPath()).latin1());
#endif
...
void tst_QSettings::ctor()
{
    ...
    {
        // QSettings's default constructor is always native.
        if (format == QSettings::NativeFormat) {
...
void tst_QSettings::testErrorHandling()
{
    ...
    system(QString("chmod 700 %1 2>/dev/null").arg(settingsPath("someDir")).latin1());
    system(QString("chmod -R u+rwx %1 2>/dev/null").arg(settingsPath("someDir")).latin1());
    system(QString("rm -fr %1").arg(settingsPath("someDir")).latin1());
    ...
    if (filePerms != -1) {
        ...
        system(QString("mkdir -p %1").arg(settingsPath("someDir")).latin1());
    }
...
#define testVal(key, val, tp, rtype) \
    { \
        ...
        settings.setValue(key, qVariantFromValue(val)); \
    } \
    QConfFile::clearCache(); \
    { \
        QSettings settings(format, QSettings::UserScope, "software.org", "KillerAPP"); \
...
void tst_QSettings::sync()
{
    ...
    */
    ...
    unlink((userConfDir + "other.software.org.ini").latin1());
    rename((userConfDir + "software.org.ini").latin1(),
           (userConfDir + "other.software.org.ini").latin1());
...
/*
    ...
    unlink((userConfDir + "software.org.ini").latin1());
    rename((userConfDir + "other.software.org.ini").latin1(),
            (userConfDir + "software.org.ini").latin1());
