...
void tst_QSystemSemaphore::basicProcesses()
{
    ...
    QStringList acquireArguments = QStringList() << "../lackey/scripts/systemsemaphore_acquire.js";
    QProcess acquire;
    acquire.setProcessChannelMode(QProcess::ForwardedChannels);

    QStringList releaseArguments = QStringList() << "../lackey/scripts/systemsemaphore_release.js";
    QProcess release;
    release.setProcessChannelMode(QProcess::ForwardedChannels);

    acquire.start("../lackey/lackey", acquireArguments);
    QVERIFY(!acquire.waitForFinished(100));
    QVERIFY(acquire.state()== QProcess::Running);
    acquire.kill();
    release.start("../lackey/lackey", releaseArguments);
    QVERIFY(acquire.waitForFinished(100));
...
void tst_QSystemSemaphore::processes()
{
    ...
    for (int i = 0; i < processes; ++i)
        scripts.append("../lackey/scripts/systemsemaphore_acquirerelease.js");
...
void tst_QSystemSemaphore::initialValue()
{
    ...
    acquire.start("../lackey/lackey", acquireArguments << "2");
    QVERIFY(!acquire.waitForFinished(100));
    QVERIFY(acquire.state()== QProcess::Running);
    acquire.kill();
