...
QChar XQueryTokenizer::tokenizeCharacterReference()
{
    ...
    else if(content.startsWith(QLatin1Char('#')))
    {
        ...
        const QChar ch(content.toInt(&conversionOK, base));
...
Tokenizer::Token XQueryTokenizer::tokenizeStringLiteral()
{
    ...
    for(;m_pos < m_length; ++m_pos)
    {
        ...
        if(c == QLatin1Char('&'))
        {
            const QChar charRef(tokenizeCharacterReference());
        ...
        else if(c == delimiter)
        {
            ...
            else if(current() == delimiter) /* Double quote. */
                result += delimiter;
            else
                return Token(STRING_LITERAL, result);
        }
    ...
    switch(state())
    {
        ...
        case QuotAttributeContent:
        {
            ...
            while(true)
            {
                ...
                {
                    const QChar ret(tokenizeCharacterReference());
                    if(ret.isNull())
                        return Token(ERROR);
                    else
                        result.append(ret);
                ...
                switch(peekCurrent())
                {
                    case '<':
                    {
                        if(!result.isEmpty() && peekAhead(2) != '[')
                        {
                            ...
                            return Token(mayBeWS ? STRING_LITERAL : NON_BOUNDARY_WS, result);
                        }
        ...
        case ProcessingInstructionContent:
        {
            ...
            else
            {
                m_pos += 2; /* Consume "?>" */
                popState();
                return Token(PI_CONTENT, m_data.mid(start, len));
            }
            Q_ASSERT(false);
        }
        case EndTag:
        ...
        case XMLComment:
        {
            ...
            else
            {
                ...
                // TODO error reporting can be better
                if(peekCurrent() == '>')
                {
                    ++m_pos;
                    return Token(COMMENT_CONTENT, m_data.mid(start, len));
                }
                else
                    return error();
            }
