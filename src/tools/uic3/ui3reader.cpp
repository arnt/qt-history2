...
void Ui3Reader::registerLayouts(const QDomElement &e)
{
...
void Ui3Reader::createActionDecl(const QDomElement& e)
{
...
void Ui3Reader::createToolbarDecl(const QDomElement &e)
{
    if (e.tagName() == "toolbar")
        out << "    " << "QToolBar *" << getObjectName(e) << ";" << endl;
}

void Ui3Reader::createMenuBarDecl(const QDomElement &e)
{
    if (e.tagName() == "item") {
...
void Ui3Reader::createPopupMenuDecl(const QDomElement &e)
{
    for (QDomElement n = e.firstChild().toElement(); !n.isNull(); n = n.nextSibling().toElement()) {
        if (n.tagName() == "action") {
...
void Ui3Reader::createActionImpl(const QDomElement &n, const QString &parent)
{
    for (QDomElement ae = n; !ae.isNull(); ae = ae.nextSibling().toElement()) {
        QString objName = registerObject(getObjectName(ae));
        QString clsName = QLatin1String("QAction");
        if (ae.tagName() == "action") {
            out << indent << objName << " = new QAction(" << parent << ");" << endl;
            out << indent << objName << "->setObjectName(\"" << objName << "\");" << endl;
        } else if (ae.tagName() == "actiongroup") {
            clsName = QLatin1String("QActionGroup");
            out << indent << objName << " = new QActionGroup(" << parent << ");" << endl;
            out << indent << objName << "->setObjectName(\"" << objName << "\");" << endl;
        } else
            continue;
        ...
        for (QDomElement n2 = ae.firstChild().toElement(); !n2.isNull(); n2 = n2.nextSibling().toElement()) {
            if (n2.tagName() == "property") {
                ...
                if (prop == QLatin1String("name"))
                    continue;
                else if (prop == QLatin1String("menuText"))
                    prop = "text";
                else if (prop == QLatin1String("accel"))
                    prop = "shortcut";
                else if (prop == QLatin1String("iconSet"))
                    prop = "icon";

                if (prop == QLatin1String("text")) {
                    if (hasText)
                        continue;
                    hasText = true;
                }
                ...
                if (!WidgetInfo::isValidProperty(clsName, prop))
                    continue;
                ...
                if (stdset) {
                    ...
                    call += "setProperty(\"" + prop + "\", QCoreVariant(" + value + "));";
                }
        ...
        if (!hasText && !actionText.isEmpty() && uiFileVersion < "3.3")
            trout << indent << objName << "->setText(" << actionText << ");" << endl;
    }
}
...
void Ui3Reader::createToolbarImpl(const QDomElement &n, const QString &parentClass, const QString &parent)
{
    ...
    for (int i = 0; i < (int) nl.length(); i++) {
        ...
        for (QDomElement n2 = ae.firstChild().toElement(); !n2.isNull(); n2 = n2.nextSibling().toElement()) {
            if (n2.tagName() == "action") {
                out << indent << objName << "->addAction(" << n2.attribute("name") << ");" << endl;
...
void Ui3Reader::createMenuBarImpl(const QDomElement &n, const QString &parentClass, const QString &parent)
{
    QString objName = getObjectName(n);
    out << indent << objName << " = new QMenuBar(this);" << endl;
    out << indent << objName << "->setObjectName(\"" << objName << "\");" << endl;
    ...
    while (!c.isNull()) {
        if (c.tagName() == "item") {
            ...
            out << endl;
            out << indent << objName << "->addSeparator(" << i << ");" << endl;
        }
...
void Ui3Reader::createPopupMenuImpl(const QDomElement &e, const QString &parentClass, const QString &parent)
{
    int i = 0;
    for (QDomElement n = e.firstChild().toElement(); !n.isNull(); n = n.nextSibling().toElement()) {
        if (n.tagName() == "action") {
            ...
            if (n2.tagName() == "item") { // the action has a sub menu
                ...
                out << indent << itemName << " = new QPopupMenu(this);" << endl;
                out << indent << parent << "->insertItem("  << n.attribute("name") << "->icon(), ";
                ...
                out << indent << parent << "->addAction(" << n.attribute("name") << ");" << endl;
            }
        } else if (n.tagName() == "separator") {
            out << indent << parent << "->addSeparator();" << endl;
        }
        ++i;
    }
}
...
QString Ui3Reader::createListBoxItemImpl(const QDomElement &e, const QString &parent,
                                    QString *value)
{
    ...
    while (!n.isNull()) {
        if (n.tagName() == "property") {
            ...
            QCoreVariant v = DomTool::elementToVariant(n.firstChild().toElement(), QCoreVariant());
            if (attrib == "text") {
                txt = v.toString();
                com = getComment(n);
...
QString Ui3Reader::createIconViewItemImpl(const QDomElement &e, const QString &parent)
{
    QDomElement n = e.firstChild().toElement();
    QString txt;
    QString com;
...
QString Ui3Reader::createListViewItemImpl(const QDomElement &e, const QString &parent,
...
QString Ui3Reader::createListViewColumnImpl(const QDomElement &e, const QString &parent,
                                       QString *value)
{
    QDomElement n = e.firstChild().toElement();
    QString txt;
...
QString Ui3Reader::createTableRowColumnImpl(const QDomElement &e, const QString &parent,
                                       QString *value)
{
 ...
 */
QString Ui3Reader::createLayoutImpl(const QDomElement &e, const QString& parentClass, const QString& parent, const QString& layout)
{
    QDomElement n;
...
QString Ui3Reader::createSpacerImpl(const QDomElement &e, const QString& /*parentClass*/, const QString& /*parent*/, const QString& /*layout*/)
{
    QDomElement n;
    QString objClass, objName;
    objClass = e.tagName();
    ...
    Size size = asVariant(DomTool::readProperty(e, "sizeHint", QCoreVariant(QCoreVariant::Invalid, &defaultSize))).size;
    ...
    if (sizeType != "Expanding" && sizeType != "MinimumExpanding" &&
        ...
        Rect defaultRect;
        defaultRect.init(0, 0, 0, 0);
        Rect geom = asVariant(DomTool::readProperty(e, "geometry", QCoreVariant(QCoreVariant::Invalid, &defaultRect))).rect;
 ...
 */
QString Ui3Reader::createObjectInstance(const QString& objClass, const QString& parent, const QString& objName)
{
    Q_UNUSED(objName);
