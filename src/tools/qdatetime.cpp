...
/*
  We are in qt/tools, so we are not allowed to use QObject::tr().
  Instead, we do this:

   1. Provide QDate::set{Short,Long}{Month,Day}Names() to override
      the English strings.
   2. Use QT_TRANSLATE_NOOP so that the English strings show up in
      the translation file generated by lupdate.
   3. Call QDate::set{Short,Long}{Month,Day}Names() in
      QApplication::{install,remove}Translator() with the
      translated version of the strings.

  The following four variables are global so that QApplication can
  use them.
*/
...
extern const char * const qt_shortMonthNames[] = {
    QT_TRANSLATE_NOOP("QDate::shortMonth", "Jan"),
    QT_TRANSLATE_NOOP("QDate::shortMonth", "Feb"),
    QT_TRANSLATE_NOOP("QDate::shortMonth", "Mar"),
    QT_TRANSLATE_NOOP("QDate::shortMonth", "Apr"),
    QT_TRANSLATE_NOOP("QDate::shortMonth", "May"),
    QT_TRANSLATE_NOOP("QDate::shortMonth", "Jun"),
    QT_TRANSLATE_NOOP("QDate::shortMonth", "Jul"),
    QT_TRANSLATE_NOOP("QDate::shortMonth", "Aug"),
    QT_TRANSLATE_NOOP("QDate::shortMonth", "Sep"),
    QT_TRANSLATE_NOOP("QDate::shortMonth", "Oct"),
    QT_TRANSLATE_NOOP("QDate::shortMonth", "Nov"),
    QT_TRANSLATE_NOOP("QDate::shortMonth", "Dec")
};

extern const char * const qt_longMonthNames[] = {
    QT_TRANSLATE_NOOP("QDate::longMonth", "January"),
    QT_TRANSLATE_NOOP("QDate::longMonth", "February"),
    QT_TRANSLATE_NOOP("QDate::longMonth", "March"),
    QT_TRANSLATE_NOOP("QDate::longMonth", "April"),
    QT_TRANSLATE_NOOP("QDate::longMonth", "May"),
    QT_TRANSLATE_NOOP("QDate::longMonth", "June"),
    QT_TRANSLATE_NOOP("QDate::longMonth", "July"),
    QT_TRANSLATE_NOOP("QDate::longMonth", "August"),
    QT_TRANSLATE_NOOP("QDate::longMonth", "September"),
    QT_TRANSLATE_NOOP("QDate::longMonth", "October"),
    QT_TRANSLATE_NOOP("QDate::longMonth", "November"),
    QT_TRANSLATE_NOOP("QDate::longMonth", "December")
};

extern const char * const qt_shortDayNames[] = {
    QT_TRANSLATE_NOOP("QDate::shortDay", "Mon"),
    QT_TRANSLATE_NOOP("QDate::shortDay", "Tue"),
    QT_TRANSLATE_NOOP("QDate::shortDay", "Wed"),
    QT_TRANSLATE_NOOP("QDate::shortDay", "Thu"),
    QT_TRANSLATE_NOOP("QDate::shortDay", "Fri"),
    QT_TRANSLATE_NOOP("QDate::shortDay", "Sat"),
    QT_TRANSLATE_NOOP("QDate::shortDay", "Sun")
};

extern const char * const qt_longDayNames[] = {
    QT_TRANSLATE_NOOP("QDate::longDay", "Monday"),
    QT_TRANSLATE_NOOP("QDate::longDay", "Tuesday"),
    QT_TRANSLATE_NOOP("QDate::longDay", "Wednesday"),
    QT_TRANSLATE_NOOP("QDate::longDay", "Thursday"),
    QT_TRANSLATE_NOOP("QDate::longDay", "Friday"),
...
/*!
  ...
  the toString(), shortDayName(), longDayName(), shortMonthName() and longMonthName()
  functions.
...
#endif
    if ( qt_tr_shortMonthNames )
	return (*qt_tr_shortMonthNames)[month - 1];
    else
	return QString::fromLatin1( qt_shortMonthNames[month - 1] );
}
...
#endif
    if ( qt_tr_longMonthNames )
	return (*qt_tr_longMonthNames)[month - 1];
    else
...
#endif
    if ( qt_tr_shortDayNames )
	return (*qt_tr_shortDayNames)[weekday - 1];
    else
	return QString::fromLatin1( qt_shortDayNames[weekday - 1] );
}
...
#endif
    if ( qt_tr_longDayNames )
	return (*qt_tr_longDayNames)[weekday - 1];
    else
	return QString::fromLatin1( qt_longDayNames[weekday - 1] );
}

/*!
  Sets the short month names to replace the default "Jan", "Feb", ...,
  "Dec" strings. \a names must be a list of 12 strings in that order.

  The names are used by shortMonthNames() and toString().

  \sa setShortMonthNames() setShortDayNames() setLongMonthNames() setLongDayNames()
*/

void QDate::setShortMonthNames( const QStringList& names )
{
    if ( names.count() == 12 ) {
	if ( !qt_tr_shortMonthNames ) {
	    qt_tr_shortMonthNames = new QStringList;
	    cleanup_tr_names.add( qt_tr_shortMonthNames );
	}
	*qt_tr_shortMonthNames = names;
    }
}

/*!
  Sets the short day names to replace the default "Mon", "Tue", ...,
  "Sun" strings. \a names must be a list of 7 strings in that order.
...
void QDate::setShortDayNames( const QStringList& names )
{
    if ( names.count() == 7 ) {
	if ( !qt_tr_shortDayNames ) {
	    qt_tr_shortDayNames = new QStringList;
	    cleanup_tr_names.add( qt_tr_shortDayNames );
	}
	*qt_tr_shortDayNames = names;
    }
}

/*!
  Sets the long month names to replace the default "January", "February", ...,
  "December" strings. \a names must be a list of 12 strings in that order.
...
void QDate::setLongMonthNames( const QStringList& names )
{
    if ( names.count() == 12 ) {
	if ( !qt_tr_longMonthNames ) {
	    qt_tr_longMonthNames = new QStringList;
	    cleanup_tr_names.add( qt_tr_longMonthNames );
	}
	*qt_tr_longMonthNames = names;
    }
}

/*!
  Sets the long day names to replace the default "Monday", "Tuesday", ...,
  "Sunday" strings. \a names must be a list of 7 strings in that order.
...
void QDate::setLongDayNames( const QStringList& names )
{
    if ( names.count() == 7 ) {
	if ( !qt_tr_longDayNames ) {
	    qt_tr_longDayNames = new QStringList;
	    cleanup_tr_names.add( qt_tr_longDayNames );
	}
...
QDate QDate::fromString( const QString& s, Qt::DateFormat f )
{
    ...
    case Qt::TextDate:
	{
		...
		if ( monthName == qt_shortMonthNames[i] ||
		     (qt_tr_shortMonthNames &&
		      monthName == (*qt_tr_shortMonthNames)[i]) ) {
		    month = i + 1;
		    break;
		}
	    }
