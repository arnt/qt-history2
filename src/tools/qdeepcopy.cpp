...
/*!
  ...
  Normally, shared copies reference the same data in memory as a
  memory and speed optimization.  In the example below,
  \c s1, \c s2, \c s3, \c s4 and \c s5 share data.
  ...
  unique, unshared data.  In the example below, \c s1, \c s2 and \c s5
  share data, while neither \c s3 nor \c s4 share data.
  ...
  In the example below, \c s1, \c s2 and \s5 share data, and \c s3 and \c s4
  share data.
  ...
  use shared classes.  In the example below, the variable \c global_string
  is used safely since the data contained in \c global_string is always
  a deep copy.  This ensures that all threads get a unique copy of the data,
  and that any assignments to \c global_string will result in a deep copy.
