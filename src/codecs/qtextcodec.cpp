...
static QArray<unsigned char> * reverseMap = 0;
#endif
...
void QSimpleTextCodec::buildReverseMap()
{
    ...
    if ( reverseOwner != this ) {
        ...
        m++;
        if ( !reverseMap )
            reverseMap = new QArray<unsigned char>( m );
...
#else
    if ( !reverseMap ) {
	QArray<unsigned char> **map = &((QSimpleTextCodec *)this)->reverseMap;
        int m = 0;
        int i = 0;
        while( i < 128 ) {
            if ( unicodevalues[forwardIndex].values[i] > m &&
        ...
        m++;
	*map = new QArray<unsigned char>( m );
        for( i = 0; i < 128 && i < m; i++ )
            (**map)[i] = (char)i;
        for( ;i < m; i++ )
            (**map)[i] = 0;
