...
public:
    ...
    uint ro: 1;
    QCoreVariant::Type type;
    QSqlField::Status req;
    int len;
    int prec;
    QCoreVariant def;
    int tp;
    QSqlField::Status gen;
};
...
/*!
    ...
    type. If the field is required \a required must not be -1; the
    field's length is given by \a fieldLength, and its precision (only
    meaningful for numeric types) is given by \a prec. The field's
    default value is given by \a defValue, its SQL type by \a sqlType
    (which must be 0), and if it is auto-generated \a autogen must not
    be -1.
*/
...
void QSqlField::setRequired(Status required)
{
    detach();
    d->req = required;
}
...
void QSqlField::setAutoGenerated(Status autogen)
{
    detach();
    d->gen = autogen;
}
...
*/
QSqlField::Status QSqlField::isRequired() const
{
    return d->req;
}
...
/*!
    ...
    This is set in the constructor.
*/
QSqlField::Status QSqlField::isAutoGenerated() const
{
    return d->gen;
}
