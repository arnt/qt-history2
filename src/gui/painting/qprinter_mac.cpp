...
public:
    ...
    virtual bool begin(const QPaintDevice *pdev);
    virtual bool end();
};
bool
QPrinterPaintEngine::begin(const QPaintDevice *pdev)
{
    if(!print->printerBegin())
        return false;
    return QWrapperPaintEngine::begin(const_cast<QPaintDevice *>(pdev));
}
bool
