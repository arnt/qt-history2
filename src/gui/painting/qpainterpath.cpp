 ...
 * \class QPainterPathStroker
 *
 * \brief The QPainterPathStroker class is used by QPen to convert
 * paths into fillable outlines.
 *
 * This class is primarly used by QPen to create outlines that can be
 * drawn. This is required because in vector graphics because the
 * outline of a shape does not have any dimensions, as it is an
 * infinitly thin path. The stroker iterates trough the path and
 * expands the path to an area representing the path at a given
 * width. This area can then be filled resulting in that the outline
 * is drawn.
 *
 * The stroker also handles cap styles using setCapStyle(), join
 * styles using setJoinStyle() and dash patterns using
 * setDashPattern(). The dimension of the created outline is specified
 * using setWidth().
 *
 * The stroker also gives finegrain control over more advanced features
 * compared to QPen, such as custom dash patterns and miterlimit.
 *
 * The outline path generated by createStroke() should only be used
 * for filling as outline of the outlines usually have self
 * intersections which may be viewed as drawing artifacts. Generated
 * outlines also require the winding fill rule which is set by
 * default.
 *
 * \sa QPen, QBrush
 */
...
/*!
    Creates a new path that is a fillable area representing the
    outline of \a path based on the properties in the stroker.

    The stroke has a winding fill rule (which is required) and should
    only be filled, not stroked using QPen.
*/
...
/*!
    Sets the width of the generated outline to \width. The generated
    outlines will extend 50% of \a width to each side of the input
    path.
*/
...
/*!
    ...
    dash pattern is in effect each segment is subject to the current
    dash style.
*/
...
/*!
    Sets the miter limit of the generated outlines to \a limit. The
    miter limit describes how far from each join the miter join can
    extend. The limit is specified in the units of width so the
    pixelwise miter limit will be \i { miterlimit * width }.
...
/*!
    Sets the curve flattening threshold for the generated
    outlines. This value has a good default and should not need to be
    modified.
*/
...
/*!
    ...
    Sets the dash pattern to for generated outlines to \a style.
*/
...
/*!
    Sets the dash pattern for the generated outlines to \a dashPattern.

    This function makes it possible to specify custom dash patterns.

    The cap style is applied to each dash segment. The pen style is
    represented units of width so a dot line of width 20 will have
    dots that are 20 pixels tall and wide.
*/
