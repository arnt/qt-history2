                                 ...
                                 const QStyleOption& opt) const
{
    ...
    case CE_ProgressBarContents: {
        ...
        switch (qt_aqua_size_constrain(pbar)) {
        case QAquaSizeUnknown:
        case QAquaSizeLarge:
            ttdi.kind = pbar->totalSteps() ? kThemeLargeProgressBar
                                           : kThemeLargeIndeterminateBar;
            break;
        case QAquaSizeMini:
#if (MAC_OS_X_VERSION_MAX_ALLOWED >= MAC_OS_X_VERSION_10_3)
            if (QSysInfo::MacintoshVersion >= QSysInfo::MV_PANTHER) {
                ttdi.kind = pbar->totalSteps() ? kThemeMiniProgressBar
                                               : kThemeMiniIndeterminateBar;
                break;
            }
#endif
        case QAquaSizeSmall:
            ttdi.kind = pbar->totalSteps() ? kThemeMediumProgressBar
                                        ...
                                        const QStyleOption& opt) const
{
    ...
    case CC_SpinBox: {
        ...
        if(sub & SC_SpinBoxFrame)
            drawPrimitive(PE_PanelLineEdit, p, querySubControlMetrics(CC_SpinBox, sw, SC_SpinBoxFrame),
                          pal, Style_Sunken);
        if((sub & SC_SpinBoxDown) || (sub & SC_SpinBoxUp)) {
    ...
    case CC_Slider: {
	    ...
	    int numTicks = (sldr->maximum() - sldr->minimum() + 1) / sldr->pageStep();
                                            ...
                                            const QStyleOption& opt) const
{
    ...
    case CC_Slider: {
        ...
        case SC_SliderHandle: {
            RgnHandle rgn = qt_mac_get_rgn();
            GetThemeTrackThumbRgn(&ttdi, rgn);
            GetRegionBounds(rgn, &mrect);
            QRect ret = QRect(mrect.left, mrect.top, (mrect.right - mrect.left) + 1,
...
QStyle::SubControl QMacStyleQD::querySubControl(ComplexControl cc, const Q4StyleOptionComplex *opt,
