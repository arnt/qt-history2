...
void QMacStyleQDPainter::setport()
{
    ...
    if(d_ptr->engine && (d_ptr->engine->type() == QPaintEngine::QuickDraw || d_ptr->engine->type() == QPaintEngine::CoreGraphics))
                                     ...
                                     QPainter *p, const QWidget *widget) const
{
    ...
    case CC_ComboBox:
        ...
        break;
    case CC_TitleBar:
        if (const Q4StyleOptionTitleBar *tbar = qt_cast<const Q4StyleOptionTitleBar *>(opt)) {
        ThemeWindowMetrics twm;
        memset(&twm, 0, sizeof(twm));
        twm.metricSize = sizeof(twm);
