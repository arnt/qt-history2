...
void QFileDialog::setAcceptMode(QFileDialog::AcceptMode mode)
{
    ...
    // ### TODO setStandardButtons needs a Choose for directories
...
void QFileDialogPrivate::updateFileTypeVisibility()
{
    ...
    if (fileTypeCombo->count() == 1
        && fileTypeCombo->itemText(0) == QFileDialog::tr("AllFiles (*)"))
            showFilterGUI = false;
    fileTypeCombo->setVisible(showFilterGUI && acceptMode == QFileDialog::AcceptSave);
    fileTypeLabel->setVisible(showFilterGUI && acceptMode == QFileDialog::AcceptSave);
}
...
QStringList QFileDialog::history() const
{
    ...
    currentHistory << d->model->filePath(d->rootIndex());
                                    ...
                                    Options options)
{
    ...
    if (qt_use_native_dialogs && !(args.options & DontUseNativeDialog)) {
        ...
        for (int i = 0; i < files.count(); ++i)
            files.replace(i, QUnicodeTables::normalize(files.at(i), QString::NormalizationForm_C));
        return files;
    }
#endif
...
void QFileDialog::accept()
{
    ...
    // special case for ".."
    if (d->fileNameEdit->text() == QLatin1String("..")) {
...
void QFileDialogPrivate::_q_updateOkButton() {
    ...
    case QFileDialog::AnyFile:
        button->setEnabled(!model->isDir(index) || !fileNameEdit->text().isEmpty());
    }
}
