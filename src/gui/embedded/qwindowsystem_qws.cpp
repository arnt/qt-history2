...
/*!
    ...
    This enum describes the various types of mouse events used when
...
/*!
    ...
    Reimplement the filter() function to intercept key events from
    physical or virtual keyboards, and use the setInputResolution()
    function to control the number of bits shifted to go from pointer
    resolution to screen resolution when filtering mouse events. The
    current settings can be retrieved using the inputResolutionShift()
    function. To handle mouse events within a preedit text,
    QWSInputMethod provides the virtual mouseHandler() function.
    ...
    The QWSInputMethod class also provides functions for
    sending and receiving responses to input method queries,
    sendQuery() and queryResponse().
...
/*!
    ...
    The events are specified by the given \a type which is a value of
    the UpdateType enum. Note that reimplementations of this function
    must call the base implementation for all cases that it does not
    handle itself.
...
/*!
  ...
  mouse events within a preedit text.

  Internally, preedit texts are passed encapsulated by an \l
  {QWSEvent::IMEvent}{IMEvent} generated by the sendPreeditString()
  function. The \a offset specifies the position of the mouse event
  within the string, and \a state the type of event as described by
  the QWSServer::IMMouse event. If \a state is less than 0, the mouse
  event is inside the associated widget, but outside the preedit
  text. When clicking in a different widget, the \a state is
...
/*!
  ...
  Sends an input method query (internally encapsulated by an \l
  {QWSEvent::IMQuery}{IMQuery} event) for the specified \a property.
...
/*!
    ...
    coordinates do not match the device pointer coordinates.
    ...
    cause the event being filtered by the current input method.
