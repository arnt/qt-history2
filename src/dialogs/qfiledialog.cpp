...
void QFileListBox::viewportMousePressEvent( QMouseEvent *e )
{
    ...
    QFileDialog::Private::MCItem *i1 = (QFileDialog::Private::MCItem*)item( currentItem() );
    if ( i1 )
	mousePressed = !( (QFileDialog::Private::File*)i1->i )->info.isDir();
...
QFileDialog::QFileListView::QFileListView( QWidget *parent, QFileDialog *dlg )
...
void QFileDialog::QFileListView::setSorting( int column, bool increasing )
{
    if ( column == -1 ) {
...
void QFileDialog::QFileListView::changeSortColumn2( int column )
{
    int lcol = header()->mapToLogical( column );
    setSorting( lcol, sortcolumn == lcol ? !ascending : TRUE );
}

void QFileDialog::QFileListView::keyPressEvent( QKeyEvent *e )
{
    if ( ( e->key() == Key_Enter ||
	   e->key() == Key_Return ) &&
	 renaming )
...
void QFileDialog::QFileListView::viewportMousePressEvent( QMouseEvent *e )
{
    pressPos = e->pos();
    mousePressed = FALSE;
    ...
    QFileDialog::Private::File *i1 = (QFileDialog::Private::File*)currentItem();
    if ( i1 )
...
void QFileDialog::QFileListView::viewportMouseDoubleClickEvent( QMouseEvent *e )
{
    renameTimer->stop();
    QListView::viewportMouseDoubleClickEvent( e );
}

void QFileDialog::QFileListView::viewportMouseReleaseEvent( QMouseEvent *e )
{
...
void QFileDialog::QFileListView::viewportMouseMoveEvent( QMouseEvent *e )
{
    renameTimer->stop();
    if ( !dragItem )
	dragItem = itemAt( e->pos() );
...
void QFileDialog::QFileListView::dragObjDestroyed()
{
#ifndef QT_NO_DRAGANDDROP
    //######
    //filedialog->rereadDir();
...
#ifndef QT_NO_DRAGANDDROP
void QFileDialog::QFileListView::viewportDragEnterEvent( QDragEnterEvent *e )
{
    startDragUrl = filedialog->d->url;
    startDragDir = filedialog->dirPath();
    currDropItem = 0;
...
void QFileDialog::QFileListView::viewportDragMoveEvent( QDragMoveEvent *e )
{
    if ( acceptDrop( e->pos(), e->source() ) ) {
	if ( oldDragPos != e->pos() )
	    setCurrentDropItem( e->pos() );
...
void QFileDialog::QFileListView::viewportDragLeaveEvent( QDragLeaveEvent * )
{
    changeDirTimer->stop();
    setCurrentDropItem( QPoint( -1, -1 ) );
//########
...
void QFileDialog::QFileListView::viewportDropEvent( QDropEvent *e )
{
    changeDirTimer->stop();
...
bool QFileDialog::QFileListView::acceptDrop( const QPoint &pnt, QWidget *source )
{
...
void QFileDialog::QFileListView::setCurrentDropItem( const QPoint &pnt )
{
    changeDirTimer->stop();
...
void QFileDialog::QFileListView::changeDirDuringDrag()
{
#ifndef QT_NO_DRAGANDDROP
    if ( !currDropItem )
	return;
...
void QFileDialog::QFileListView::doubleClickTimeout()
{
    startRename();
    renameTimer->stop();
}

void QFileDialog::QFileListView::startRename( bool check )
{
...
void QFileDialog::QFileListView::clear()
{
    cancelRename();
    QListView::clear();
}

void QFileDialog::QFileListView::rename()
{
    if ( !lined->text().isEmpty() ) {
...
void QFileDialog::QFileListView::cancelRename()
{
    renameItem = 0;
    lined->parentWidget()->hide();
    viewport()->setFocusProxy( this );
...
void QFileDialog::QFileListView::contentsMoved( int, int )
{
    changeDirTimer->stop();
#ifndef QT_NO_DRAGANDDROP
    setCurrentDropItem( QPoint( -1, -1 ) );
...
QFileDialog::Private::File::~File()
{
    if ( d->pendingItems.findRef( this ) )
	d->pendingItems.removeRef( this );
}

QString QFileDialog::Private::File::text( int column ) const
{
    makeVariables();
...
const QPixmap * QFileDialog::Private::File::pixmap( int column ) const
{
    if ( column ) {
	return 0;
...
QFileDialog::Private::MCItem::MCItem( QListBox * lb, QListViewItem * item )
    : QListBoxItem()
{
    i = item;
    if ( lb )
...
QFileDialog::Private::MCItem::MCItem( QListBox * lb, QListViewItem * item, QListBoxItem *after )
    : QListBoxItem()
{
    i = item;
    if ( lb )
...
QString QFileDialog::Private::MCItem::text() const
{
    return i->text( 0 );
}


const QPixmap *QFileDialog::Private::MCItem::pixmap() const
{
    return i->pixmap( 0 );
}


int QFileDialog::Private::MCItem::height( const QListBox * lb ) const
{
    if ( pixmap() )
...
int QFileDialog::Private::MCItem::width( const QListBox * lb ) const
{
...
void QFileDialog::Private::MCItem::paint( QPainter * ptr )
{
...
void QFileDialog::fileNameEditReturnPressed()
{
	...
	if ( nameEdit->text().isEmpty() ) {
	    ...
	    QUrlInfo f;
	    Private::File * c
		= (Private::File *)files->currentItem();
...
QStringList QFileDialog::selectedFiles() const
{
	...
	while( i ) {
	    if ( i->isSelected() ) {
		QUrl u = QUrl( d->url, ((Private::File*)i)->info.name() );
		if ( u.isLocalFile() ) {
		    QString s = u.toString();
		    if ( s.left( 5 ) == "file:" )
			s.remove( 0, 5 );
...
void QFileDialog::updateFileNameEdit( QListViewItem * newItem )
{
	...
	Private::File * i = (Private::File *)newItem;
	if ( !i->i->selected() ) {
...
void QFileDialog::detailViewSelectionChanged()
{
	...
	if ( d->moreFiles && isVisible() ) {
	    if ( ( (Private::File *)i )->i->selected() != i->isSelected() )
		d->moreFiles->setSelected( ( (Private::File *)i )->i, i->isSelected() );
	}
	if ( i->isSelected() && !( (Private::File *)i )->info.isDir() )
	...
	QUrl u = QUrl( d->url, ((Private::File*)files->currentItem())->info.name() );
	if ( d->infoPreviewer )
	    d->infoPreviewer->previewUrl( u );
	if ( d->contentsPreviewer )
	    d->contentsPreviewer->previewUrl( u );
...
void QFileDialog::listBoxSelectionChanged()
{
    ...
    while( i ) {
	if ( files && isVisible() ) {
	    if ( ( (Private::MCItem *)i )->i->isSelected() != i->selected() )
		files->setSelected( ( (Private::MCItem *)i )->i, i->selected() );
	}
	if ( d->moreFiles->isSelected( i )
	&& !( (Private::File*)( (Private::MCItem *)i )->i )->info.isDir() ) {
	...
	QUrl u = QUrl( d->url,
		       ( (Private::File*)( (Private::MCItem*)j )->i )->info.name() );
	if ( d->infoPreviewer )
	    d->infoPreviewer->previewUrl( u );
	if ( d->contentsPreviewer )
	    d->contentsPreviewer->previewUrl( u );
...
void QFileDialog::updateFileNameEdit( QListBoxItem * newItem )
{
    if ( !newItem )
	return;
    Private::MCItem * i = (Private::MCItem *)newItem;
    if ( d->mode != ExistingFiles ) {
					   ...
					   const QString& caption )
{
	...
	*workingDirectory = QDir::currentDirPath();
	...
	while( i ) {
	    if ( i->isSelected() ) {
		QUrl u = QUrl( dlg->d->url, ((Private::File*)i)->info.name() );
		if ( u.isLocalFile() ) {
		    QString s = u.toString();
		    if ( s.left( 5 ) == "file:" )
			s.remove( 0, 5 );
...
void QFileDialog::fixupNameEdit()
{
	...
	if ( ( (Private::File*)files->currentItem() )->info.isFile() )
...
void QFileDialog::insertEntry( const QValueList<QUrlInfo> &lst, QNetworkOperation *op )
{
	...
	if ( !d->url.isLocalFile() ) {
	    Private::File * i = 0;
	    Private::MCItem *i2 = 0;
	    i = new Private::File( d, &inf, files );
	    i2 = new Private::MCItem( d->moreFiles, i );
...
void QFileDialog::removeEntry( QNetworkOperation *op )
{
	...
	if ( ( (Private::File*)it.current() )->info.name() == op->arg( 0 ) ) {
	    d->pendingItems.removeRef( (Private::File*)it.current() );
	    delete ( (Private::File*)it.current() )->i;
	    delete it.current();
	    ok1 = TRUE;
	}
...
void QFileDialog::itemChanged( QNetworkOperation *op )
{
	...
	if ( ( (Private::File*)it1.current() )->info.name() == op->arg( 1 ) ) {
	    delete ( (Private::File*)it1.current() )->i;
	    delete it1.current();
	    ok1 = TRUE;
	}
    ...
    for ( i = d->sortedList.first(); it.current(); ++it, i = d->sortedList.next() ) {
	if ( ( (Private::File*)it.current() )->info.name() == op->arg( 0 ) ) {
	    ( (Private::File*)it.current() )->info.setName( op->arg( 1 ) );
	    ok1 = TRUE;
	}
	if ( i && i->name() == op->arg( 0 ) ) {
	    i->setName( op->arg( 1 ) );
...
void QFileDialog::resortDir()
{
	...
	item = new Private::File( d, i, files );
	item2 = new Private::MCItem( d->moreFiles, item, item2 );
	item->i = item2;
...
void QFileDialog::doMimeTypeLookup()
{
    ...
    QRect r;
    Private::File *item = d->pendingItems.first();
    if ( item ) {
	QFileInfo fi;
	if ( d->url.isLocalFile() ) {
