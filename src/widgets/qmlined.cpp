/**********************************************************************
** $Id: //depot/qt/main/src/widgets/qmlined.cpp#41 $
**
...
void QMultiLineEdit::paintCell( QPainter *painter, int row, int )
{
    ...
    p.setPen( g.text() );
    p.drawText( BORDER,  yPos, width() - BORDER, cellHeight( row ),
		ExpandTabs, *s );
    if ( markX1 != markX2 ) {
	...
	if ( markX2 == sLength )
	    fillxpos2 += 3;
	p.setClipping( TRUE );
	p.setClipRect( fillxpos1, 0, fillxpos2 - fillxpos1, cellHeight(row) );
