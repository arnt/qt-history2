/****************************************************************************
** $Id: //depot/qt/main/src/widgets/qpushbutton.cpp#30 $
**
...
#if defined(DEBUG)
static char ident[] = "$Id: //depot/qt/main/src/widgets/qpushbutton.cpp#30 $";
#endif
...
void QPushButton::drawButton( QPainter *paint )
{
    ...
    bool     use_pm = FALSE;			// use pixmap update
	...
	w = x2 + 1;
	h = y2 + 1;
	pmkey.sprintf( "$qt_push_%d_%d_%d_%d_%d_%d", gs,
		       palette().serialNumber(),
		       isDown(), defButton, w, h );
	pm = QPixmapCache::find( pmkey );
	if ( pm ) {				// pixmap exists
	    p->drawPixmap( 0, 0, *pm );
	    drawButtonFace( p );
	    lastDown = isDown();
