...
/*!
  ...
  limitations regarding their contents.  Everything that can be painted can
  also be stored in a picture, e.g. fonts, pixmaps, regions, transformed
  graphics, etc.
...
/*!
  ...
  The \a formatVersion parameter may be used to \e create a QPicture that can
  ...
  \i \a formatVersion == 4 is binary compatible with Qt 3.x.
  \endlist

  Note that the default formatVersion is -1 which signifies the
  current release, i.e. for Qt 3.0 a formatVersion of 4 is the same as
  the default formatVersion of -1.
