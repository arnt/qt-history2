...
extern bool qt_old_transformations; //qwmatrix.cpp
/*!
  \internal
*/
QRegion operator*(const QWMatrix &mat, const QRegion &r)
{
    if ( mat.isIdentity() )
	return r;
	...
	if ( mat.m12() == 0.0F && mat.m21() == 0.0F ) {
	    // simple case, no rotation
	    while ( i ) {
		int x = qRound( mat.m11()*rect->x() + mat.dx() );
		int y = qRound( mat.m22()*rect->y() + mat.dy() );
		int w = qRound( mat.m11()*rect->width() );
		int h = qRound( mat.m22()*rect->height() );
		if ( w < 0 ) {
		    w = -w;
		    x -= w-1;
		}
...
\internal
*/
QRegion operator*(const QWMatrix &mat, const QRect &rect)
{
    QRegion result;
    if ( mat.isIdentity() ) {
	result = rect;
    } else if ( mat.m12() == 0.0F && mat.m21() == 0.0F ) {
	if( qt_old_transformations ) {
	    result = QRect( mat.map(rect.topLeft()), mat.map(rect.bottomRight()) ).normalize();
	} else {
	    int x = qRound( mat.m11()*rect.x() + mat.dx() );
	    int y = qRound( mat.m22()*rect.y() + mat.dy() );
	    int w = qRound( mat.m11()*rect.width() );
	...
	result = QRegion(mat.mapToPolygon(rect));
    }
    return result;
