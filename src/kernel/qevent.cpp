...
/*!
  ...
  In general, it should be used as a value, just like an integer or long.  It 
  contains two integer members that you can access directly, miType contains the
  type of the device and miPhyId contains an id from the device.  These two values
  combined are guarenteed to be unique.

  Note: Unfortunately the implementation of the SGI and Windows drivers are 
  different enough that the Ids generated by the same device on SGI and Windows 
  will be different.  The miType value will be different, but the miPhyId should
  remain the same.
*/
