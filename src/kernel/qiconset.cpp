...
enum IconOrigin { Supplied, Manufactured, Derived };
...
/*! \class QIconSet qiconset.h
  ...
  is 32x32. These sizes only affect generated icons.
...
QPixmap QIconSet::pixmap( Size size, Mode mode, State state ) const
{
    ...
    if ( state == Off ) {
	if ( mode == Disabled &&
	     d->icon(this, size, Normal, Off)->origin != Derived ) {
	    icon->pixmap = createDisabled( size, Off );
	} else if ( otherSizeIcon->origin != Derived ) {
	...
	if ( mode == Normal ) {
	    if ( otherSizeIcon->origin != Derived ) {
		icon->pixmap = createScaled( size, otherSizeIcon->pixmap );
	    } else {
	    ...
	    if ( offIcon->origin != Derived ) {
		icon->pixmap = new QPixmap( *offIcon->pixmap );
	    } else if ( d->icon(this, size, Normal, On)->origin != Derived ) {
		icon->pixmap = createDisabled( size, On );
	    } else if ( otherSizeIcon->origin != Derived ) {
		icon->pixmap = createScaled( size, otherSizeIcon->pixmap );
	    } else if ( otherSizeOffIcon->origin != Derived ) {
...
/*!
  ...
  A pixmap obtained from a QIconFactory is considered as generated.
*/
bool QIconSet::isGenerated( Size size, Mode mode, State state ) const
{
    if ( !d )
	return TRUE;
    return d->icon( 0, size, mode, state )->origin != Supplied;
}
