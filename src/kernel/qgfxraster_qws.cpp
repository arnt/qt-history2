...
void QScreenCursor::init(SWCursorData *da, bool init)
{
    ...
    imgunder = new QImage( data->under, 64, 64, gfx->bitDepth(), 0,
		gfx->bitDepth() == 1 ? 0 : 256, QImage::LittleEndian );
...
void QScreenCursor::saveUnder()
{
	...
	gfxunder->setSourceOffset(x,y);
	gfxunder->blt(0,0,data->width,data->height);
	qt_sw_cursor = TRUE;
    } else {
	// This is faster than the above - at least until blt is
	// better optimised.
...
{
    int oldx = srcoffs.x() - srcwidgetx;
    int oldy = srcoffs.y() - srcwidgety;
    srcwidgetx = x;
    srcwidgety = y;
    setSourceOffset( oldx, oldy );
}

void QGfxRasterBase::setMasking(bool on,int color)
{
    ismasking=on;
    maskcol=color;
    alphatype=IgnoreAlpha;
...
#define GET_MASKED(rev) \
			...
			if( (alphatype==LittleEndianMask && !rev) || \
			    (alphatype==BigEndianMask && rev) ) { \
			    if(amonobitval & 0x1) { \
				masked=FALSE; \
			    } \
						    ...
						    bool reverse)
{
	...
	if ( !ismasking ) {
		...
		while(w--) {
		    if (srctype==SourceImage)
			gv = get_value_32(srcdepth,&srcdata,FALSE);
		    bool masked = TRUE;
		    GET_MASKED(FALSE);
		    if ( masked )
			myptr++;
	...
	if(!ismasking) {
		...
		if(reverse) {
			...
			*(myptr--)=get_value_16(srcdepth,&srcdata,TRUE);
		    myptr--;
		    while ( count-- ) {
			put = get_value_16(srcdepth,&srcdata,TRUE);
			put = (put<<16)|get_value_16(srcdepth,&srcdata,TRUE);
		...
		while( count-- ) {
		    if(srctype==SourceImage)
			gv=get_value_16( srcdepth, &srcdata, TRUE);
		    bool masked=TRUE;
		    GET_MASKED(TRUE);  //### does not work in reverse!!!
		    if(masked) {
			myptr--;
		...
		while( count-- ) {
		    if(srctype==SourceImage)
			gv=get_value_16( srcdepth, &srcdata, FALSE);
		    bool masked=TRUE;
		    GET_MASKED(FALSE);
		    if(masked) {
			myptr++;
	...
	if(!ismasking) {
		...
		while( count-- ) {
		    if(srctype==SourceImage)
			gv=get_value_8( srcdepth, &srcdata, FALSE);
		    bool masked=TRUE;
		    GET_MASKED(FALSE);
		    if(masked) {
			myptr++;
...
#define FASTPATH_BLIT
	...
	for (; j!=tj; j+=dj,ry+=dry,l+=dl) {
		...
		if (plot) {
		    ...
		    srcptr += (x-rx+srcoffs.x())*bytesPerPixel;
	...
	for (; j!=tj; j+=dj,ry+=dry,l+=dl) {
		...
		if (plot) {
		    unsigned char *srcptr=srcScanLine(j+srcoffs.y());
		    if ( reverse )
			srcptr += (x2-rx+srcoffs.x())*srcdepth/8;
		    else
			srcptr += (x-rx+srcoffs.x())*srcdepth/8;
		    if(srctype==SourceImage && srcdepth==1)
			srcptr=find_pointer(srcScanLine(0),(x-rx)+srcoffs.x(),
...
{
    char *name;
    QScreen *(*qt_get_screen)(int, const char *, char *, unsigned char *);
} driverTable [] = {
    { "LinuxFb", qt_get_screen_linuxfb },
#if !defined(QT_NO_QWS_MACH64)
    { "Mach64", qt_get_screen_mach64 },
#endif
#if !defined(QT_NO_QWS_VOODOO3)
    { "Voodoo3", qt_get_screen_voodoo3 },
#endif
#if !defined(QT_NO_QWS_VFB)
    { "QVFb", qt_get_screen_qvfb },
#endif
#if !defined(QT_NO_QWS_VNC)
    { "VNC", qt_get_screen_vnc },
#endif
    { 0,         0 },
};

QScreen *qt_get_screen( int display_id, const char *spec, char *slot, unsigned char *config )
{
	...
	if ( QString( driverTable[i].name ) == driver )
	    return driverTable[i].qt_get_screen( display_id, spec, slot, config );
	i++;
    }

    qFatal( "No driver %s", driver.latin1() );
